...
void getControllerInfos() {
  ...
  Serial.print("{[success]Hello from the Twin Audio Visualizer with PixelLED support!}\n");
  Serial.printf("{[info][Device] Firmware: %s}\n", VERSION);
  Serial.printf("{[info][Device] Serialnumber: %s}\n", teensySN());
  Serial.printf("{[info][Device] MAC Address: %s}\n", teensyMAC());
  Serial.printf("{[info][Device] UID 64-bit: %s}\n", teensyUID64());
  Serial.printf("{[info][Device] CPU Clock Speed: %i MHz}\n", F_CPU_ACTUAL / 1000000);
  Serial.printf("{[warning][Device] CPU CoreTemp: %.1f [degrees]}\n", tempmonGetTemp());
